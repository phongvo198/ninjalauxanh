...
public class RotationLuck extends Thread {
    ...
    protected synchronized void joinLuck(final User p, final int joinAmount) throws IOException {
        ...
        if (this.total > this.maxTotal) {
            p.session.sendMessageLog("Số lượng " + currency + " tối đa làaaa " + util.getFormatNumber(this.maxTotal));
            return;
        }
        Players p2 = null;
        for (short i = 0; i < this.players.size(); ++i) {
        ...
        if (p2 == null && (joinAmount > this.max || joinAmount < this.min)) {
            p.session.sendMessageLog("Bạn chỉ có thể đặt cược từ aaaaa" + util.getFormatNumber(this.min) + " đến " + util.getFormatNumber(this.max) + currency + ".");
            return;
        }
        if (p2 == null) {
            p2 = new Players(p.id);
        ...
        if (p2.joinAmount + joinAmount > this.max) {
            p.session.sendMessageLog("Bạn chỉ có thể đặt tối đaaaa " + util.getFormatNumber(this.max - p2.joinAmount) + ".");
            return;
        }
